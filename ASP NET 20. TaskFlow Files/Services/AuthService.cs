using ASP_NET_20._TaskFlow_Files.Config;
...
public class AuthService : IAuthService
{
    ...
    private async Task<AuthResponseDto> GenerateTokenAsync(ApplicationUser user)
    {
        ...
        var (refreshToken, jwt) = await CreateRefreshTokenJwtAsync(user.Id, _config.RefreshTokenExpirationInDays);

        return new AuthResponseDto
        {
            Email = user.Email!,
            AccessToken = tokenString,
            ExpiresAt = DateTime.UtcNow.AddMinutes(_config.ExpirationInMinutes),
            RefreshToken = jwt,
